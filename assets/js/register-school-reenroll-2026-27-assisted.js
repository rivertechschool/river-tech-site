...
(function () {
  ...
  const HOUSEHOLD_FEE = 0;   // Assisted form: no enrollment fee (special situations).
  ...
  function submitForm(e) {
    ...
    if (!BACKEND_URL || BACKEND_URL === "__BACKEND_URL__") {
      ...
      showError("Almost ready — the backend isn't connected yet. Your details are complete. Please try again in a few minutes, or email learn@rivertech.me to re-enroll by hand.");
    ...
    fetch(BACKEND_URL, {
      ...
      .then(function (data) {
        if (data && data.ok) {
          window.location.href = "register-school-reenroll-2026-27-assisted-success.html";
        } else {
